...
static inline void LV_DRV_DISP_SPI_CS(bool hi);
static inline void LV_DRV_DISP_CMD_DATA(bool dc);
static inline void LV_DRV_DISP_RST(bool hi);
static inline void LV_DRV_DELAY_US(uint64_t us);
static inline void LV_DRV_DELAY_MS(uint32_t ms);
static inline void LV_DRV_DISP_SPI_WR_BYTE(uint8_t byte);
static inline void LV_DRV_DISP_SPI_WR_ARRAY(uint8_t* data, uint16_t len);
/**********************
 *  STATIC VARIABLES
 **********************/
...
void ili9341_init(void)
{
    ...
    initialize_ports(); // Initialize pico ports
...
static inline void LV_DRV_DISP_CMD_DATA(bool dc) {gpio_put(ili_cfg.dc, dc);}

static inline void LV_DRV_DISP_RST(bool hi) {gpio_put(ili_cfg.resx, hi); 
}
static inline void LV_DRV_DELAY_US(uint64_t us) {sleep_us(us);}
inline void LV_DRV_DELAY_MS(uint32_t ms) {sleep_ms(ms);}

static inline void LV_DRV_DISP_SPI_WR_BYTE(uint8_t byte) {
    spi_write_blocking(ili_cfg.iface, &byte, 1);
}
static inline void LV_DRV_DISP_SPI_WR_ARRAY(uint8_t* data, uint16_t len) {
    spi_write_blocking(ili_cfg.iface, data, len);
}
